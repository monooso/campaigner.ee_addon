...
(function($) {
	...
	/**
	 * Retrieves the 'clients list' HTML via AJAX.
	 *
	 * @return	void
	 */
	function getClients() {
		...
		$.get(
			ajaxUrl,
			{ajax_request : 'y', addon_id : addonId, api_key : apiKey, action : 'get_clients'},
	...
	function getLists() {
		...
		if ( ! clientId) {
			alert(languageStrings.missingClientId);
			$('#client_id').focus();
			return;
		}
		...
		$.get(
			ajaxUrl,
			{ajax_request : 'y', addon_id : addonId, api_key : apiKey, client_id : clientId, action : 'get_lists'},
	 ...
	 * @return	void
	 */
	function handleGetClientsResponse(html) {
		$('#clients').html(html);
		$('#lists').empty();
	...
	/**
	 * Hijacks the 'Get Clients' and 'Get Mailing Lists' links.
	 *
	 * @return	void
	 */
	function iniAjaxLinks() {
		$('#sjl .ajax.button').bind('click', function(e) {
			if (this.id == 'get-clients') {
				getClients();
			} else if (this.id == 'get-lists') {
				getLists();
			}
		});
		
		$('#sjl .ajax.button').bind('keydown', function(e) {
